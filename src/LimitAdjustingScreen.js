...
const LimitAdjustingScreen = ({ onSubmit }) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:9090/api/counterparties')
      .then(response => setAllBranches(response.data))
...
// };
  ...
  const filteredBranches = allBranches.filter(item => {
    const key = searchType === 'name' ? item.cpName : item.cpId;
    return key.toLowerCase().includes(counterparty.toLowerCase());
  });

  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f5f5' }}>
      {/* Left Sidebar */}
      <div style={{ width: '30%', background: '#e6e6e6', padding: '3rem', borderRight: '1px solid #ccc' }}>
        <h3 style={{ fontFamily: 'monospace', color: '#2e8b57' }}>Limit Creating Screen</h3>
            ...
            {/* {filteredBranches.map(branch => (
              <div key={branch.cpId}>
                <label>
                  <input
                    type="checkbox"
                    checked={selectedBranches.some(b => b.cpId === branch.cpId)}
                    onChange={() => handleBranchSelect(branch)}
                  />
                  {branch.cpName}
                </label>
